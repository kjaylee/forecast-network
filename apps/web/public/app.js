...
function forecastCard(forecast) {
  ...
  return `<article class="forecast-row"><div class="row-meta"><span class="category">${esc(categoryLabel(forecast.category))}</span><a href="/creators/${encodeURIComponent(forecast.creator?.id || '')}" data-link class="creator">${avatar(forecast.creator?.displayName)}${esc(forecast.creator?.displayName || t('ui.creator'))}</a><span aria-hidden="true">·</span>${stateBadge(forecast)}</div><h2 class="row-title"><a href="${link}" data-link>${translationText(forecast,forecast.title?'title':'question',forecast.title || forecast.question)}</a></h2>${translationControl(forecast)}${participationNotice(forecast)}<div class="probability-row"><div class="probability-main"><strong>${crowd===null?'—':Math.round(crowd)}${crowd===null?'':`<span class="percent">%</span>`}</strong><span>${crowd===null?t('ui.awaitingForecasts'):t('ui.yesProbability')}</span></div><div class="probability-secondary"><span>${esc(t('ui.topForecasters'))}<b>${percentage(forecast.top?.probability)}</b></span><span>${esc(t('quality.experts'))}<b>${percentage(forecast.expert?.probability)}</b></span><span>AI<b>${percentage(forecast.ai?.probability)}</b></span></div></div><div class="probability-track" role="img" aria-label="${crowd===null?t('ui.noCrowdYet'):`${t('ui.crowdProbability',{probability:formatNumber(Math.round(crowd))})}`}"><span data-probability="${crowd ?? 0}"></span></div>${qualityReasonMarkup(forecast)}<div class="row-bottom"><div class="row-counts"><span>${icon('people')}${formatNumber(count)}</span><a href="${link}#comments" data-link aria-label="${esc(t('ui.commentsCount',{count:formatNumber(Number(forecast.commentCount)||0)}))}">${icon('comment')} ${Number(forecast.commentCount)||0}</a><button class="text-icon" data-action="share" data-id="${esc(forecast.id)}" data-title="${esc(forecast.title || forecast.question)}" aria-label="${esc(t('ui.shareForecast'))}">${icon('share')}</button></div>${forecastIsOpen(forecast)?`<div class="row-choices"><a class="choice yes" href="${link}?choose=YES#cast" data-link>YES</a><a class="choice" href="${link}?choose=NO#cast" data-link>NO</a></div>`:`<a class="link-arrow row-status-link" href="${link}" data-link>${forecast.finalizedOutcome?t('ui.viewOutcome'):t('ui.viewProgress')}${icon('arrow')}</a>`}</div></article>`;
}
...
function renderFeedItems(data,explore,append=false){
  ...
  const reasons=new Map((data.dailyRecommendations||[]).map(item=>[item.id,item.reason]));
  const markup=(data.items||[]).map(item=>forecastCard({...item,recommendationReason:reasons.get(item.id)})).join('');
...
function comparisonMarkup(forecast){return `<div class="comparison comparison-four" aria-label="${esc(t('ui.compareProbabilities'))}">${[[t('ui.crowd'),forecast.crowd,t('ui.participants',{count:formatNumber(forecast.crowd?.count || 0)})],[t('ui.topForecasters'),forecast.top,forecast.top?.count?t('ui.participants',{count:formatNumber(forecast.top.count)}):t('ui.requiresRecord')],[t('quality.experts'),forecast.expert,forecast.expert?.count?t('ui.participants',{count:formatNumber(forecast.expert.count)}):t('quality.noExperts')],[t('ui.aiForecast'),forecast.ai,forecast.ai?.count?forecast.ai.provider:t('ui.noEstimate')]].map(([label,group,caption])=>{const number=probability(group?.probability);return `<div class="comparison-item"><div class="comparison-label">${esc(label)}</div><div class="comparison-value">${number===null?'—':`${formatNumber(Math.round(number))}<small>%</small>`}</div><div class="comparison-caption">${esc(caption)}</div><div class="probability-track"><span data-probability="${number ?? 0}"></span></div></div>`;}).join('')}</div><details class="quality-explanation cohort-explanation"><summary>${esc(t('quality.whoQualifies'))}</summary><p>${esc(t('quality.expertRules'))}</p></details>${qualityReasonMarkup(forecast)}`;}
function qualityReasonMarkup(forecast){
  const quality=forecast.quality;if(!quality||!Number.isFinite(quality.componentsBp?.clarity))return '';
  const creator=quality.creatorSampleStatus==='established'?'establishedCreator':quality.creatorSampleStatus==='provisional'?'provisionalCreator':'newCreator';
  return `<details class="quality-explanation"><summary>${esc(t('quality.whyQuestion'))}</summary>${forecast.recommendationReason==='clear-cold-start'?`<p>${esc(t('quality.freshQuestion'))}</p>`:''}<p>${esc(t('quality.rankingHint'))}</p><dl class="quality-breakdown"><div><dt>${esc(t('quality.clarity'))}</dt><dd>${percentage(quality.componentsBp.clarity/100)}</dd></div><div><dt>${esc(t('quality.creatorRecord'))}</dt><dd>${esc(t('quality.'+creator))}</dd></div></dl><p>${esc(t('quality.engagementCap'))}</p></details>`;
}
function reputationQualityMarkup(rep){
  if(!rep?.methodologyVersion)return '';
  const ratio=value=>Number.isFinite(value)?percentage(value*100):'—';
  const status=rep.consistency?.status==='established'?'establishedRecord':rep.consistency?.status==='provisional'?'provisionalRecord':'newRecord';
  const entries=Array.isArray(rep.expertise)?rep.expertise:[];
  const windows=rep.consistency?.windows || [];
  const metrics=[[t('quality.consistency'),ratio(rep.consistencyScore)],[t('quality.calibration'),ratio(rep.calibrationScore)],[t('quality.creatorQuality'),ratio(rep.creatorQuality)],[t('quality.disputeAccuracy'),percentage(rep.disputeAccuracy)]];
  return `<section class="profile-section quality-record"><h2>${esc(t('quality.forecastQuality'))}</h2><dl class="quality-metrics">${metrics.map(([label,value])=>`<div><dt>${esc(label)}</dt><dd>${esc(value)}</dd></div>`).join('')}</dl><p class="quality-sample-status">${esc(t('quality.'+status))}</p><p class="quiet-note">${esc(t('quality.consistencyHint'))} ${rep.consistency?.status==='established'?'':esc(t('quality.consistencyNeeds'))}</p>${windows.length?`<p class="quiet-note">${esc(t('quality.windowSamples',{counts:windows.map(window=>formatNumber(window.count||0)).join(' / ')}))}</p>`:''}<h3>${esc(t('quality.categoryRecord'))}</h3>${entries.length?`<ul class="expertise-records">${entries.map(entry=>{const qualified=entry.status==='qualified'&&entry.qualified===true;const key=qualified?'qualified':entry.status==='not-qualified'?'notQualified':entry.status==='new'?'new':'provisional';return `<li><div class="expertise-heading"><strong>${esc(categoryLabel(entry.category))}</strong><span class="quality-status${qualified?' is-qualified':''}">${esc(t('quality.'+key))}</span></div><p>${esc(t('quality.sampleCount',{count:formatNumber(entry.count||0),minimum:formatNumber(entry.minimumSamples||20)}))}</p>${entry.count?`<p>${esc(t('quality.expertMeasures',{brier:Number.isFinite(entry.brierScore)?formatNumber(entry.brierScore,{maximumFractionDigits:3}):'—',calibration:ratio(entry.calibrationScore)}))}</p>`:''}</li>`;}).join('')}</ul>`:`<p class="quiet-note">${esc(t('quality.noCategory'))}</p>`}<details class="quality-explanation"><summary>${esc(t('quality.qualityMethod'))}</summary><p>${esc(t('quality.qualityMethodHint'))}</p><p>${esc(t('quality.expertRules'))}</p></details>${Number.isFinite(rep.asOf)?`<p class="quality-asof">${esc(t('quality.asOf',{date:date(rep.asOf,true)}))}</p>`:''}</section>`;
}
...
const seekerState={busy:false,error:'',request:null};
function seekerRequestCurrent(request=seekerState.request){
  return request!==null&&request===seekerState.request&&request.owner===state.user?.id&&request.epoch===pointsState.epoch&&request.sequence===state.sequence;
}
function seekerMarkup(){
  const seeker=state.me?.seeker;
  const current=seekerRequestCurrent();const busy=current&&seekerState.busy;
  if(!seeker?.available)return '';
  const wallet=state.me?.authentication?.method==='wallet';
  const status=seeker.status;
  const verified=status?`<div class="wallet-status seeker-status"><strong>${icon('shield')}${esc(t('ui.seekerVerified',{number:status.memberNumber??'—'}))}</strong><span>${esc(t('ui.seekerSkr',{amount:formatNumber(Number(status.skr)||0,{maximumFractionDigits:2})}))}</span></div>`:'';
  const action=!wallet?`<p class="quiet-note">${esc(t('ui.seekerNeedsWallet'))}</p>`:button(esc(t(status?'ui.seekerRefresh':'ui.seekerVerify')),'seeker-verify',status?'quiet small':'secondary',busy?'disabled':'');
  return `<p class="quiet-note">${esc(t('ui.seekerIntro'))}</p>${verified}${busy?`<p class="loading-label">${esc(t('ui.seekerVerifying'))}</p>`:action}<p class="error-message" role="alert">${esc(current?seekerState.error:'')}</p>`;
}
function renderSeeker(){const node=document.querySelector('#seeker-content');if(node)node.innerHTML=seekerMarkup();}
async function verifySeeker(){
  if((seekerState.busy&&seekerRequestCurrent())||!state.user)return;
  const request={owner:state.user.id,epoch:pointsState.epoch,sequence:state.sequence};
  seekerState.request=request;seekerState.busy=true;seekerState.error='';renderSeeker();
  try{
    const status=await api('/api/seeker/verify',{method:'POST',body:{expectedUserId:request.owner}});
    if(!seekerRequestCurrent(request))return;
    if(state.me)state.me.seeker={...state.me.seeker,status};
    toast(t('ui.seekerVerifiedToast'));
  }catch(error){
    if(seekerRequestCurrent(request)){
      if(['seeker_not_found','seeker_verification_changed'].includes(error.code)&&state.me?.seeker)state.me.seeker.status=null;
      seekerState.error=errorText(error);
    }
  }
  finally{if(seekerRequestCurrent(request)){seekerState.busy=false;renderSeeker();}}
}
...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    else if(action==='seeker-verify'){await verifySeeker();return;}
  if(action==='share-profile')await shareProfile();
      ...
      const sequence=state.sequence;const owner=state.user?.id;const epoch=pointsState.epoch;
      const path=location.pathname;const search=location.search;
      const current=()=>sequence===state.sequence&&owner===state.user?.id&&epoch===pointsState.epoch&&path===location.pathname&&search===location.search;
      target.disabled=true;const params=new URLSearchParams(search);params.set('cursor',target.dataset.cursor);
      try{const data=await api(`/api/forecasts?${params}`);if(current())renderFeedItems(data,path==='/explore',true);}
...
document.addEventListener('submit',async event=>{
  ...
  if(form.id==='comment-form'){
    let sequence=state.sequence;const forecastId=routeId();
    const initiatingOwner=state.user?.id;const initiatingEpoch=pointsState.epoch;
    if(!await ensureAuth())return;
    if(!state.user||forecastId!==routeId()||(initiatingOwner&&(sequence!==state.sequence||initiatingOwner!==state.user.id||initiatingEpoch!==pointsState.epoch)))return;
    // Successful sign-in repaints the same detail before ensureAuth resumes.
    sequence=state.sequence;
    const owner=state.user?.id;const epoch=pointsState.epoch;
    const current=()=>sequence===state.sequence&&forecastId===state.detail?.forecast?.id&&owner===state.user?.id&&epoch===pointsState.epoch;
    await withForm(form,'comment-error',async()=>{
      if(!current())return;
      const text=new FormData(form).get('text').trim();if(!text)throw uiError('ui.enterComment');
      try{
        const data=await api(`/api/forecasts/${encodeURIComponent(forecastId)}/comments`,{method:'POST',body:{text},idempotent:true});
        if(!current())return;
        const list=document.querySelector('#comment-list');
        if(list){
          const comments=state.detail.comments||[];
          state.detail.comments=comments.some(comment=>comment.id===data.comment.id)?comments:[...comments,data.comment];
          list.innerHTML=commentsMarkup(state.detail.comments);
        }else{
          // Sign-in may still be loading a detail snapshot from before the POST.
          const repaint=renderRoute({preserve:true});sequence=state.sequence;
          await repaint;if(!current())return;
        }
        if(form.isConnected)form.reset();toast(t('ui.commentPosted'));
      }catch(error){if(current())throw error;}
    },t('ui.posting'));return;
  }
