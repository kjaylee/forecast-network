...
const seekerState={busy:false,error:''};
function seekerMarkup(){
  ...
  const action=!wallet?`<p class="quiet-note">${esc(t('ui.seekerNeedsWallet'))}</p>`:button(esc(t(status?'ui.seekerRefresh':'ui.seekerVerify')),'seeker-verify',status?'quiet small':'secondary',seekerState.busy?'disabled':'');
  return `<p class="quiet-note">${esc(t('ui.seekerIntro'))}</p>${verified}${seekerState.busy?`<p class="loading-label">${esc(t('ui.seekerVerifying'))}</p>`:action}<p class="error-message" role="alert">${esc(seekerState.error)}</p>`;
}
...
async function verifySeeker(){
  if(seekerState.busy||!state.user)return;
  seekerState.busy=true;seekerState.error='';renderSeeker();
  try{
    const status=await api('/api/seeker/verify',{method:'POST',body:{expectedUserId:state.user.id}});
    if(state.me)state.me.seeker={...state.me.seeker,status};
    toast(t('ui.seekerVerifiedToast'));
  }catch(error){seekerState.error=errorText(error);}
  finally{seekerState.busy=false;renderSeeker();}
}
