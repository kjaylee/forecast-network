...
def main() -> None:
    ...
    store = mark(1024, 0.60, background=NAVY)
    store.save(ASSETS / "icon-only.png")
    mark(1024, 0.60).save(ASSETS / "icon-foreground.png")
    Image.new("RGBA", (1024, 1024), NAVY).save(ASSETS / "icon-background.png")
    for density, size in DENSITIES.items():
        folder = RES / f"mipmap-{density}"
        folder.mkdir(exist_ok=True)
        # Adaptive layers: the XML insets both by 16.7%, so the visible 72dp circle shows this
        # whole image; the ring stays inside the 66% safe zone.
        mark(size, 0.78).save(folder / "ic_launcher_foreground.png")
        Image.new("RGBA", (size, size), NAVY).save(folder / "ic_launcher_background.png")
        legacy = mark(size, 0.60, background=NAVY)
