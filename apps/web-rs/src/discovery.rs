...
fn tie_coefficients(as_of_ms: i64, user_id: Option<&str>) -> Vec<i64> {
    // serde_json emits raw UTF-8, and the Python side passes ensure_ascii=False for the same
    // reason: the two used to disagree for a non-ASCII user id, and the coefficients are
    // interpolated into SQL, so the same database could be ordered two ways. Account ids are
    // `u_` plus base64url and always ASCII, which is why nothing ever showed it.
...
mod tests {
    ...
    /// Generated by `forecast_application.discovery.tie_break`, which is the implementation the
    /// Python Worker uses and the one the SQL expression mirrors. Discovery order must not depend
    /// on which of the three answered, and nothing was checking that until now.
    #[test]
    fn tie_break_matches_the_python_implementation() {
        let cases: &[(&str, i64, Option<&str>, i64)] = &[
            ("f_BHbIOkOLWiMNIEGD3sjxrOSf", 1_800_000_000_000, None, 275_240),
            ("f_BHbIOkOLWiMNIEGD3sjxrOSf", 1_800_000_000_000, Some("user-a"), 314_780),
            ("f_tkqvmogdeeITz0c_XMNJTB68", 1_800_086_400_000, None, 274_888),
            ("f_tkqvmogdeeITz0c_XMNJTB68", 1_800_086_400_000, Some("user-b"), 386_187),
        ...
        for (identifier, as_of_ms, user_id, expected) in cases {
            assert_eq!(
                tie_break(identifier, *as_of_ms, *user_id),
                *expected,
                "tie_break disagrees with Python for {identifier:?} at {as_of_ms} as {user_id:?}"
            );
        }
    }
    ...
    #[test]
    fn the_sql_expression_carries_the_same_weights_as_the_rust_one() {
        let (day, user) = (1_800_000_000_000, Some("user-a"));
        let sql = tie_sql(day, user, "f.id");
        let weights: Vec<i64> = tie_coefficients(day, user);
        assert_eq!(sql.matches("COALESCE(unicode(substr(f.id,").count(), weights.len());
        for (index, weight) in weights.iter().enumerate() {
            let term = format!("COALESCE(unicode(substr(f.id,{},1)),0)*{}", index + 1, weight);
            assert!(sql.contains(&term), "SQL is missing {term} from {sql}");
        }
        assert!(sql.starts_with('(') && sql.ends_with(')'));
    }

    /// D1 hands integral numbers back through JavaScript as floats, so 7 arrives looking like 7.0.
    #[test]
    fn integral_floats_from_the_database_count_as_integers() {
        assert_eq!(integer(&json!(7)), Some(7));
        assert_eq!(integer(&json!(7.0)), Some(7));
        assert_eq!(integer(&json!(-3.0)), Some(-3));
        assert_eq!(integer(&json!(7.5)), None, "a fractional count is not a count");
    ...
    #[test]
    fn a_missing_or_negative_count_is_refused_rather_than_treated_as_zero() {
        let mut row = serde_json::Map::new();
        assert_eq!(count(&row, "n").unwrap(), 0, "an absent column is genuinely zero");
        row.insert("n".to_string(), json!(4));
        assert_eq!(count(&row, "n").unwrap(), 4);
        for bad in [json!(-1), json!("4"), json!(1.5)] {
            row.insert("n".to_string(), bad.clone());
            assert!(count(&row, "n").is_err(), "{bad} must be refused, not coerced");
        }
    }
}
